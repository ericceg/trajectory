...
- Advanced Analytics keeps an in-memory result cache (keyed by analytics definitions/time range plus last scan timestamp), so revisiting the page feels instant when nothing changed; recomputation only runs when definitions/range/data version change or when you press `Recompute`
