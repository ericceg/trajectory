...
- Activity detail chart hover can highlight the corresponding GPS position on the route map with a moving dot (when sample coordinates are available)
...
5. Explore:
   ...
   - Hover **Activity Detail** charts to see the matching point on the route map highlighted with a moving dot (when GPS samples exist)
